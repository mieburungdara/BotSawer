...
class Bot
{
    ...
    private function handleMediaUpload($message): void
    {
        try {
            ...
            $isFirstInGroup = $mediaData['is_first_in_group'];

            if (!$isDuplicate && $isFirstInGroup) {
                // Only run these for new uploads (and only once per album)
    ...
    private function saveMediaToDatabase($botId, $userId, array $mediaInfo): array
    {
        ...
        $existing = DB::table('media_files')
            ->join('contents', 'media_files.content_id', '=', 'contents.id')
            ->where('media_files.file_unique_id', $mediaInfo['file_unique_id'])
            ->where('contents.user_id', $userId)
            ->select('media_files.id as media_id', 'contents.short_id', 'contents.status', 'contents.id as content_id')
            ->first();
        ...
        if ($existing) {
            ...
            return ['id' => $existing->media_id, 'short_id' => $existing->short_id, 'is_duplicate' => true, 'is_first_in_group' => false];
        }
        ...
        if (!empty($mediaInfo['media_group_id'])) {
            $existingContent = DB::table('contents')
                ->where('media_group_id', $mediaInfo['media_group_id'])
                ->where('user_id', $userId)
                ->first();
            
            if ($existingContent) {
                $contentId = $existingContent->id;
                $shortId = $existingContent->short_id;
        ...
        if (!$contentId) {
            ...
            $contentId = DB::table('contents')->insertGetId([
                ...
                'media_group_id' => $mediaInfo['media_group_id'] ?? null,
                'status' => 'draft'
            ]);
        }
        ...
        return ['id' => $mediaId, 'short_id' => $shortId, 'is_duplicate' => false, 'is_first_in_group' => $isFirstInGroup];
    }
